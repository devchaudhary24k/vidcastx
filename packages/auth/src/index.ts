...
export interface InitAuthOptions {
  ...
  secret: string;
  /** Origins allowed to initiate OAuth and receive post-auth redirects. */
...
/**
 * Build a Better Auth instance with VidcastX's shared schema, hooks, and
 * organization plugin. All environment-specific values are passed in so this
 * package reads no env vars and stays framework-agnostic.
 *
 * Consumer apps (e.g. `apps/api`) call this from their own auth entry point
 * with values read from their own `env.ts`.
 */
