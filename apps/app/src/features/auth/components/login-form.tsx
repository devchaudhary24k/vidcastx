...
export function LoginForm({ className, ...props }: React.ComponentProps<"div">) {
  ...
  const form = useForm({
    ...
    onSubmit: async ({ value }) => {
      const { error } = await emailSignIn(value.email, value.password);

      if (error) {
        form.setErrorMap({ onSubmit: { form: error.message || "Failed to login", fields: {} } });
        return;
      }
    ...
    <div className={cn("flex flex-col gap-6", className)} {...props}>
      <Card>
        <CardHeader className="text-center">
          ...
          <CardDescription>Login with your GitHub or Discord account</CardDescription>
        </CardHeader>
        <CardContent>
          <form
            onSubmit={(e) => {
          ...
          >
            <FieldGroup>
              <Field>
                <Button variant="outline" type="button" onClick={() => providerSignIn("github")}>
                ...
                <Button variant="outline" type="button" onClick={() => providerSignIn("discord")}>
